...
import { BoardSettings } from "./[id]/BoardSettings";
...
/**
 * One board's kanban, rendered (specs/06-collaboration.md §2) — shared by `/boards` (the default
 * "Task board" nobody has to create) and `/boards/[id]` (any board, including custom ones), so the
 * two never drift into two slightly different kanbans.
 *
 ...
 */
export function BoardView({ boardId, allBoardsHref }: { boardId: string; allBoardsHref: string }) {
    ...
    return (
      <Card className="text-sm">
        ...
        <Button className="mt-3" variant="secondary" asChild>
          <Link href={allBoardsHref}>Back to boards</Link>
        </Button>
      </Card>
    );
  }
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="ghost" asChild>
              <Link href={allBoardsHref}>All boards</Link>
            </Button>
          </div>
        }
      />
